---
title: "# CODES - A GUIDE TO MISINFORMATION DETECTION DATASET - Keywords"
format: pdf
editor: visual
---
...
dat_claims <- read_csv("C:/Users/client/Desktop/datasets/2024-08-23_dat_claims.csv")
...
dat_claims <- dat_claims %>%
  mutate(veracity = case_when(
    ...
    label %in% c("5g covid conspiracy", "barely-true", "conspiracy", "conspiracy, fake remedy",
                 ...
                 "out-of-context", "partly true/misleading", "unlikely", "False information", "Altered photo", "False information and graphic content") ~ 2,
    label %in% c("ambiguous or hard to classify", "complicated/hard to categorise", "in dispute",
                 ...
                 "partly true", "unverified", "Missing context", "Partly false information") ~ 3,
...
COAID has no third label (other)
...
corpus <- Corpus(VectorSource(fever$claim))
corpus <- tm_map(corpus, content_transformer(tolower))
corpus <- tm_map(corpus, removePunctuation)
corpus <- tm_map(corpus, removeNumbers)
corpus <- tm_map(corpus, removeWords, stopwords("en"))
...
corpus <- Corpus(VectorSource(feverous$claim))
corpus <- tm_map(corpus, content_transformer(tolower))
corpus <- tm_map(corpus, removePunctuation)
corpus <- tm_map(corpus, removeNumbers)
corpus <- tm_map(corpus, removeWords, stopwords("en"))
...
liar <- dat_claims %>%
  filter(dataset == "liar") # Only has 2 labels
...
liar_new <- dat_claims %>%
  filter(dataset == "liar_new") # Two labels
...
confusion_matrix_random_rumors_no_filter <- table(random_predictions_rumors_no_filter, 
                                                  Y_test_rumors_no_filter)
